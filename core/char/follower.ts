...
FOLLOWER.prototype.on_teamin = function (me) {
    ...
    this.do_follow(master, true);
    if (master.environment && this.environment !== master.environment) {
        this.moveto(master.environment, this.name + "离开了。", this.name + "跟着" + master.name + "走了过来。");
    }
}
...
FOLLOWER.prototype.on_master_leave = function (me, nextrm) {
    if (!this.team || this.team != me.team) return false;
    if (this.state && this.set_state) {
        this.set_state(null, true);
    }
    ...
    if (!nextrm || !nextrm.parent || nextrm.parent.no_team) return false;

    // 入队随从跟随主人进入副本、住宅，也允许从住宅/普通地图继续跟出门。
    if (nextrm.is_fb() || nextrm.parent.id == "home" || !this.environment || !this.environment.is_fb()) {
        return true;
    }
