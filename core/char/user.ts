...
USER.prototype.query_jingli = function () {
    ...
    if (daily < 0) daily = 0;
    return daily + (this.query_temp("ad_jl", 0) || 0);
}
USER.prototype.query_jingli_text = function () {
    var expend = this.query_temp("ex_jl") || 0;
    var daily = 200 - expend;
    if (daily < 0) daily = 0;
    var added = this.query_temp("ad_jl", 0) || 0;
    var total = daily + added;
    if (added > 0) {
        return total + "（养精" + added + "/" + this.query_jclimit() + "，每日剩余" + daily + "）";
    }
