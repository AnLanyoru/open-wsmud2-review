...
FAMILY.prototype.battle_settle = function (me, suc, target_fam) {
    if (me.family !== this) return me.notify("你不是" + this.name + "弟子。");
    var battle = this.temp && this.temp["battle"];
    if (!battle) return me.notify("最近没有可领取的战利品。");
    var key = "mp_battle_reward_" + this.id;
    if (me.query_temp(key)) return me.notify("你已经领取过本次门派战利品了。");
    var time = battle.e - Date.now();
    if (!(time > 0)) time = 3600000;
    var value = 50 + Math.max(0, this.battle_gift || 0);
    if (suc > 0) value += suc;
    me.set_temp(key, 1, time);
    me.add_temp("gongji", value);
    me.notify("<hiy>你领取了本次门派战利品，获得" + value + "点门派功绩。</hiy>");
}
