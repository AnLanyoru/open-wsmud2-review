...
this.enter = function (player, target, count, objid) {
    ...
    if (target.master == player.id || target.id == player.master) {
        if (target.is_full() && !can_stack_to_target(target, obj)) return player.notify(target.name + "身上东西太多了。");
...
function can_stack_to_target(target, obj) {
    if (!obj.combined || !target.items) return false;
    for (var i = 0; i < target.items.length; i++) {
        if (target.items[i].is(obj)) return true;
    }
    return false;
}
