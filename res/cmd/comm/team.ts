...
this.team_with = function (me, user) {
    ...
    return this.add_follower_to_team(me, player);
}
this.add_follower_to_team = function (me, player) {
    if (player.master != me.id) return me.notify(player.name + "拒绝了你的组队邀请。");
    if (player.team) {
        if (player.team == me.team) {
            player.on_teamin && player.on_teamin(me);
            return me.notify(player.name + "已经在你的队伍中。");
        }
    ...
    if (player.environment && player.environment.is_fb()) return me.notify(player.name + "正在副本中。");
    if (player.state && player.set_state) {
        player.set_state(null, true);
        if (player.state) return me.notify(player.name + "正在" + player.state.title + "。");
    }
    if (me.team) {
        if (me.team.length > 4) {
    ...
    if (!me.team) {
        me.team = [];
        me.team.alloc = team_dice;
        me.team.id = UTIL.create_id();
        me.team.push(me);
    }
    me.team.push(player);
    player.team = me.team;
    player.on_teamin && player.on_teamin(me);
    me.send_team("<hig>" + player.name + "加入队伍。</hig>");
