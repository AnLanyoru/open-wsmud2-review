    ...
    if (count != undefined) {
        ...
        if (item.max > 0) {
            ...
            if (val >= item.max) {
                ...
                me.send(`{"type":"dialog","dialog":"shop",item:["${item.id}",${val}],${format_moneys(me)}}`);
            }
        }
...
const follower_gift_desc = "里面有随从契约（每个人最多可以有三个随从），少量扫荡符、天师符，最多可以购买20个，注：有几率开出你已经拥有的随从，相同随从不能同时存在";
...
this.groups = [
    [
        ...
        {
            name: "<hio>随从礼包</hio>", unit: "个", grade: 5,
            path: "cash/follower_gift", desc: follower_gift_desc, value: 2888,
            max_key: "shpm_follower_gold", max: 2
        }
    ...
    , [
        ...
        {
            name: "<hio>随从礼包</hio>", unit: "个", grade: 5,
            path: "cash/follower_gift", desc: follower_gift_desc, value: 288,
            max_key: "shpm_follower_cash", max: 20
        }
    ]
